        ...
        Piece piece, Colour colour, Board *board)
{
    ...
    if (castling_rook_col != '\0') {
        ...
        int rook_col = (class == KINGSIDE_CASTLE ? 'f' : 'd'); //Rook goes to f or d file
        /* Place the rook at its destination. */
        board->board[to_r][ColConvert(rook_col)] = MAKE_COLOURED_PIECE(colour, ROOK);
        board->weak_hash_value ^= hash_lookup(rook_col, to_rank, ROOK, colour);
    }
}
...
kingside_castle(Move *move_details, Colour colour, Board *board)
{
    ...
    if (can_castle(KINGSIDE_CASTLE, colour, board)) {
         ...
         */
        move_details->to_col = find_castling_rook_col(colour, board, KINGSIDE_CASTLE); //works for Chess960 now
...
queenside_castle(Move *move_details, Colour colour, Board *board)
{
    ...
    if (can_castle(QUEENSIDE_CASTLE, colour, board)) {
         ...
         * but is used within the program as the true target of the King..
         */
        move_details->to_col = find_castling_rook_col(colour, board, QUEENSIDE_CASTLE); //Works for Chess960 now
        move_details->to_rank = rank;
        Ok = TRUE;
    }
    else {
